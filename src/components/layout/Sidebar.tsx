...
import { LayoutDashboard, Briefcase, Users, FileText, CheckSquare, Calendar, Clock, Receipt, BarChart2, Settings, UserCog, Shield, Scale } from "lucide-react";
...
const SEC_EN: Record<string,string> = { core:"Legal Practice", billing:"Finance", manage:"Administration" };
const SEC_FR: Record<string,string> = { core:"Pratique Juridique", billing:"Finances", manage:"Administration" };

export default function Sidebar({ activePage, onNavigate, collapsed, allowedPages }: SidebarProps) {
  const { t, i18n } = useTranslation();
  const labels = i18n.language==="fr" ? SEC_FR : SEC_EN;
  let lastSection: string | null = undefined as any;

  const navItems = allowedPages
    ? ALL_NAV.filter(item => allowedPages.includes(item.key))
    : ALL_NAV;
        ...
        {navItems.map(item => {
          ...
          return (
            <React.Fragment key={item.key}>
              {showSection && !collapsed && <div className="nav-section-label">{labels[item.section!]}</div>}
              <div
